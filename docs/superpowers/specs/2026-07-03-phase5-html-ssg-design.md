...
(it is either enumerated or explicitly listed; if not, warning).

**Error policy (fail loud at build):** missing outDir → created; unwritable →
throw with path; loader throw → build fails with identity + page path;
iteration cap (§6) → build fails listing unfinished identities.
...
- `createTextNode` → next cursor child is a text node → adopt (no byte
  comparison — browser whitespace normalization; the mount pass's `setText`
  overwrites it anyway); else mismatch.
...
Assembles the full page (core module, pure string building through
`HTMLEscaping`):
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" ...>            ← default iff Page's meta lacks one
...
<body>
  …body fragment…
  <script type="module" src="…"></script>                              ← hydrate only
</body>
</html>
```
