...
- **Known output change (accepted):** today the wrapper path emits duplicate properties (`mergeStyleText` concatenates base + wrapper without cross-boundary dedupe — AttributeBag.swift:90, documented on purpose at StyledTag.swift:48-50: CSS last-wins resolves it). The typed map collapses duplicates to last-wins at build time — same visual result (outer wrapper still wins), **different serialized bytes** where wrapper and body set the same property. SSG output is therefore not byte-identical in those cases; golden/`serializeHTML` tests are updated accordingly. Per-property diffing requires this collapse (duplicate keys make the diff ambiguous). Beyond duplicate-property collapse, the tolerant parser that reads raw `style` attribute strings (`.attribute("style", …)` / `.style("prop", "val")` escape hatch) also **normalizes whitespace** — `color:red` re-serializes as `color: red` — and **drops chunks without a colon**, so malformed fragments disappear from the output rather than round-tripping verbatim.
...
Each transaction's animations register into a completion group; when all settle (or force-finish), the group's `completion` runs once. Timeout + `visibilitychange` guarantee it always fires — hidden-tab lingering ghosts and never-firing completions are the known bug class to test against (R§2.14). A transition driven by its OWN `.animation(_:)` override does not participate in any `withAnimation` completion group — it carries no `Transaction`, so it registers against no group (§3.4). Edge rules:
...
**Recorded deviation (Equatable-only):** `Animation` ships `Equatable` only — the `Sendable` conformance sketched in §3.1 was dropped per the project-wide "no Sendable in the pipeline" rule (everything is `@MainActor`; the ambient `Transaction._active` is the one sanctioned `@MainActor` static). §3.1's `public struct Animation: Equatable, Sendable` is aspirational; the shipped type is `public struct Animation: Equatable`.

**Recorded deviation (v1 known divergence — `.animation(_:value:)` × removal):** the wrapper drives ENTER transitions but NOT removal transitions. A removed identity is never resolved under the wrapper on the removing pass, so it receives no effective transaction; a plain (non-`withAnimation`) removing write also carries no ambient/default transaction. The exit therefore falls back to the transition's own `.animation(_:)` (if any) or the flush's default transaction, and with neither present the element is removed instantly. SwiftUI animates both directions. Future phase item; regression-pinned by `TransitionExitTests.animationWrapperDoesNotDriveRemoval_v1Divergence` (rename + flip when fixed).
